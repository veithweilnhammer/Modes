...
**In this work, we tested whether the phenomenon of bimodal inference, which we initially observed during bistable perception in humans[@Weilnhamer2021a], generalizes to a diverse set of two-alternative-forced-choice (2AFC) tasks in the domain of perception. Our analysis of the Confidence database[@Rahnev2020] therefore collapsed across all available perceptual 2AFC experiments, which we considered as random effects in linear mixed effects modeling. Individual experiments differed with respect to the stimuli, the manipulation of difficulty, the timing of trials, and the way responses were collected (see Method section X.X), but were comparable with respect to the central variables of stimulus- and history-congruence (Supplemental Figure SX). The variability across experiments may thus be considered as a strength of our approach, pointing to bimodal inference as a general phenomenon in 2AFC perceptual decision-making. The IBL database allowed us to not only extend our analyses to perceptual decision-making in mice, but also to replicate our findings in a highly standardized task that was collected across many individual subjects[@Aguillon-Rodriguez2020]. It will be an important task for future research to validate our results on bimodal inference in a standardized dataset of comparable volume in humans, which is, to our knowledge, not yet available.**
...
We agree with the Reviewer (see also a similar suggestion by Reviewer 1 in Comment X.X.). We have removed the section 5.8 from our manuscript and will develop a model on the potential function of between-mode fluctuations in a separate publication. 
