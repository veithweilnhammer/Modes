f_compute_group_acf <- function(PwData, acf_to_test, max_trial) {
  ...
  for (acf_idx in c(1:length(acf_to_test))){
    ...
    for (trial_idx in c(2:max_trial)){
      ...
      PwTest <- data.frame()
      Y = double()
      PwTest <- PwData[PwData$Trial == trial_idx , colnames(PwData) %in% c(paste(acf_to_test[acf_idx]), 'diff_acf_External', 'diff_acf_Difficulty', 'study_id')]
      ...
      # expr = {
      # 
      # MEM <- summary(lmer(Y ~ 1 + (1|study_id/diff_acf_Difficulty) , data = PwTest))
      # },
