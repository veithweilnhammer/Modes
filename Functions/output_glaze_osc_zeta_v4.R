output_glaze_osc_zeta_v4 <- function(par, Input_Data) {   
  ...
  for (k in c(1:n)){
    ...
    if (k == 1){
      ...
      Fit$muhat[k] = (Fit$mu[k-1] + log( ((1-H)/H) + exp(-Fit$mu[k-1])) - log( ((1-H)/H) + exp(Fit$mu[k-1])))
    }
    
    Fit$mu[k] = Fit$muhat[k]*Fit$oscillation_ei[k] + Fit$LLR[k]*Fit$oscillation_ei_LLR[k]
  ...
  Fit$accuracy_term = Fit$response*log(Fit$y_prob) + (1-Fit$response) * log(1-Fit$y_prob)
  sum_Error = (-sum(Fit$accuracy_term[is.finite(Fit$accuracy_term)]))
