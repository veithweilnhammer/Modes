simulation_glaze_no_osc <- function(outcomes, n, H, prec, amp, frequency, phase, random_steps, sliding_window, n_permutations, type) {
  ...
  Sim$oscillation_ei = (0*1.2*amp*(sin(frequency*(c(1:n)-1)))) + 1
  Sim$oscillation_ei_LLR = (0*0.4*amp*(sin(frequency*(c(1:n)-1) + phase))) + 1
