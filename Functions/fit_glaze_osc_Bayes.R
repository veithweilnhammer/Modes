fit_glaze_osc_Bayes <- function(H, prec, amp, amp_LLR, frequency, phase, Input_Data) {   
  ...
  #Fit$u = 0.5 + (Fit$env - 0.5)*(prec) ## worked previously, prec bound between 0.01 and 0.99
  #Fit$u = sigmoid((prec)*Fit$env*2 - 1)
  Fit$u = sigmoid(prec*(Fit$env - 0.5))
  ...
  accuracy_term = log(Fit$y_prob^Fit$response*(1-Fit$y_prob)^(1-Fit$response))
  sum_Accuracy = sum(accuracy_term[is.finite(accuracy_term)])
  
  #print(sum_Error)
  return(list(Score = sum_Accuracy, Pred = 0))
}