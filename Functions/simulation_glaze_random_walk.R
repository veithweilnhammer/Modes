simulation_glaze_random_walk <- function(outcomes, n, H, prec, amp, frequency, phase, random_steps, sliding_window, n_permutations, type) {
  ...
  #Sim$oscillation_ei = (amp*(sin(frequency*(c(1:n)-1) + phase)))
  ...
  for (k in c(1:n)){
    ...
    H = H + random_steps[k]
    # print(H)
    prec = prec_base - random_steps[k]

    Sim$env[k] = rbinom(n=1, size = 1, prob = 0.5)
    
    Sim$u[k] = 0.5 + (Sim$env[k] - 0.5)*prec
