simulation_glaze_osc_human_zeta_v4_one_amp_output <- function(Input_Data, outcomes, n, H, prec, amp, frequency, phase, zeta, random_steps, sliding_window, n_permutations, type) {
  ...
  Sim$oscillation_ei_LLR =  sigmoid(-amp*(sin(frequency*(c(1:nrow(Sim))-1) + phase)))/max(sigmoid(amp*(sin(frequency*(c(1:nrow(Sim))-1)))))
