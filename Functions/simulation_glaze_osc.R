simulation_glaze_osc <- function(outcomes, n, H, prec, amp, amp_LLR, frequency, phase, random_steps, sliding_window, n_permutations, type) {
  ...
  for (k in c(1:n)){
    ...
    Sim$u[k] = sigmoid((prec + random_steps[k])*Sim$env[k]*2 - 1)
