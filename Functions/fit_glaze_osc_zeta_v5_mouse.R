fit_glaze_osc_zeta_v5_mouse <- function(par, Input_Data) {   
  ...
  Fit$oscillation_ei_LLR = sigmoid(-amp_LLR*(sin(frequency*(c(1:nrow(Fit))-1) + phase)))/max(sigmoid(amp_LLR*(sin(frequency*(c(1:nrow(Fit))-1)))))
  ...
  for (k in c(1:n)){
    ...
    Fit$H[k] = sigmoid(H_logit)
    if (k == 1){
      
      Fit$muhat[k] = 0
      
    } else {
      Fit$muhat[k] = (Fit$mu[k-1] + log(((1-Fit$H[k])/Fit$H[k]) + exp(-Fit$mu[k-1])) - log( ((1-Fit$H[k])/Fit$H[k]) + exp(Fit$mu[k-1])))*Fit$oscillation_ei[k]
    }
    ...
    if (k > 1){
      Fit$p_transition[k] = abs(Fit$response[k] - Fit$response[k-1])
      Fit$da_H[k] = Fit$p_transition[k] - Fit$H[k] ## H learned based on experience
      H_logit = H_logit + alpha * Fit$oscillation_ei_LLR[k] * Fit$da_H[k]
    }
    print(H_logit)
  }
