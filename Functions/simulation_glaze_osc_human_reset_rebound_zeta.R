simulation_glaze_osc_human_reset_rebound_zeta <- function(outcomes, n, H, prec, amp, amp_LLR, frequency, phase, zeta, random_steps, sliding_window, n_permutations, type) {
  ...
  for (k in c(1:n)){
   ...
   if (accurate == 1){H = H_base} else {H = 0.5}
    # = H_base# + Sim$oscillation_ei[k]
    ...
    accurate = as.numeric((Sim$y[k] == round(Sim$u[k])))
  }
