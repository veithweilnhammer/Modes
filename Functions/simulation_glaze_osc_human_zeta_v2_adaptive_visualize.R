simulation_glaze_osc_human_zeta_v2_adaptive_visualize <- function(n_blocks, block_length, block_probs, block_precs, var_prec,
                                                        H_logit, outcomes, alpha_switch, alpha_stay, amp, frequency, zeta, 
  ...
  for (k in c(1:nrow(Sim))){
    if ( (k ==1) | (k == nrow(Sim)/2 + 1)){H_logit = H_base}
