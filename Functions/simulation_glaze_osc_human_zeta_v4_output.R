simulation_glaze_osc_human_zeta_v4_output <- function(Input_Data, outcomes, n, H, prec, amp, amp_LLR, frequency, phase, zeta, random_steps, sliding_window, n_permutations, type) {
  ...
  Sim$oscillation_ei = sigmoid(amp*(sin(frequency*(c(1:nrow(Sim))-1) + phase)))/max(sigmoid(amp*(sin(frequency*(c(1:nrow(Sim))-1)))))
  Sim$oscillation_ei_LLR =  sigmoid(-amp_LLR*(sin(frequency*(c(1:nrow(Sim))-1) + phase)))/max(sigmoid(amp_LLR*(sin(frequency*(c(1:nrow(Sim))-1)))))
  
  H_base = H
  prec_base = prec
  
  Sim$env = Input_Data$Stimulus
  max_env = which(Sim$env == max(Sim$env, na.rm = TRUE))
  min_env = which(Sim$env == min(Sim$env, na.rm = TRUE))
  Sim$env[max_env] = 1
  Sim$env[min_env] = 0
  ...
  for (k in c(1:n)){
    ...
    # Sim$env[k] = runif(1, -0.3, + 0.3) FIRSTSUCCESSFULL SIMULATION ON POSTERIOR PARAMETERS
    ...
    if (k == 1){
      ...
      Sim$muhat[k] = (Sim$mu[k-1] + log( ((1-H)/H) + exp(-Sim$mu[k-1])) - log( ((1-H)/H) + exp(Sim$mu[k-1])))
    }
