...
Optim_eval = rbind(Optim_eval[,c("value", "type", "free_param", "trial_n", "species", "convcode", "subject_id")], Optim_eval_mice[,c("value", "type", "free_param", "trial_n", "species", "convcode", "subject_id")])
Optim_eval$type = gsub("_mouse", "", Optim_eval$type)

Optim_eval$rel_value = NA
for (type_idx in unique(Optim_eval$type)){
Optim_eval[Optim_eval$type == type_idx,]$rel_value = Optim_eval[Optim_eval$type == type_idx,]$value - Optim_eval[Optim_eval$type == "fit_glaze_osc_zeta_v1_no_integration",]$value 
}
...
Sum_Subj <-   ddply(Optim_eval[Optim_eval$convcode == 0,],
                   .(subject_id, type, species),
                   summarise,
                   median_error = median(value, na.rm = TRUE),
                   sum_error = sum(value, na.rm = TRUE),
                   k = mean(free_param),
...
p_distribution_Sum <- ggplot(Optim_eval[Optim_eval$convcode == 0 & Optim_eval$type != "fit_glaze_osc_zeta_v1_one_amp",], 
...
p_distribution_Sum_Subj <- ggplot(Sum_Subj[Sum_Subj$type != "fit_glaze_osc_zeta_v1_one_amp",], 
                             aes(x = AIC,  y=..density..,  color = type, fill = type)) + 
  #geom_density(color = "white", alpha=.25, bw = 0.025, size = 0.5) + 
  geom_histogram(position = "dodge", alpha = 0.5) +
  theme_classic(base_size = 6) + xlim(-10,500) +
    scale_color_brewer(palette = "Dark2", direction = +1) + scale_fill_brewer(palette = "Dark2", direction = +1) +  
    theme(legend.position = "top") + facet_wrap(~species, ncol = 2)
p_distribution_Sum_Subj
...
Following the comment X.X by Reviewer 1, we now use the predictive coding definition of attention to explain the quadratic relationship of mode to RTs/confidence[@Feldman2010]. Based on the Bayesian formulation of drift diffusion[@Bitzer2014], we propose that the effects of likelihood on prior precision on the decision variable and, consequently, on confidence and RTs can be trasferred to mechanistic framework of drift diffusion. Specifically, we relate likelihood precision to noise in the accumulation process, and prior precision to the amount of shift in the starting point (see also our response to comment X.X of Reviewr 3). As a third response to this comment, we have re-written our discussion of the quadratic relationship of mode to RT/confidence in the light of the predictive processing definition of attention[@Feldmann2010] and its relation to mechanstic drif diffusion models. 

**In Bayesian models of perceptual decision-making, the relative precision of prior and likelihood determines their integration into the posterior that determines the content of perception. At the level of individual trials, the perceptual impact of internal predictions generated from perceptual history (prior precision) and external sensory information (likelihood precision) is thus necessarily anti-correlated. The same holds for mechanistic models of drift diffusion, which understand choice history biases as driven by changes in the starting point[@Glaze2015] or the drift rate of evidence accumulation[@Urai2019]. Under the former formulation, perceptual history is bound to have a stronger influence on perception when the less weight is given to incoming sensory evidence, assuming that the last choice is represented as a starting point bias. The effects of choice history in normative Bayesian and mechanistic drift diffusion models can be mapped onto one another via the Bayesian formulation of drift diffusion[@Bitzer2014], where the inverse of likelihood precision determines the amount of noise in the accumulation of new evidence and prior precision determines the absolute shift in its starting point[@Bitzer2014].**

Given this correspondence, we believe that we would not gain addi


Third, we have re-written our discussion of the quadratic relationship of mode to RTs and Confidence, focusing on predictive coding models attention, which we relate to mechanistic drift diffusion models: **How does attention relate between-mode fluctuations? According to predictive processing, attention corresponds to the precision afforded to the probability distributions that underlie perceptual inference[@Feldman2010]. As outlined above, between-mode fluctuations can be understood as ongoing shifts in the precision afforded to likelihood (*external mode*) and prior (*internal mode*), respectively. When the precision afforded to prior or likelihood increases, posterior precision increases, which leads to faster RTs and higher confidence. When defined from the perspective of predictive processing as the precision afforded to likelihood and prior[@Feldman2010], fluctuations in attention may thus provide a plausible explanation for the quadratic relationship of mode to RTs and confidence (Figure 2H and J; Figure 4I, Figure 6I)**.

- Fourth, we have added a more general discussion of our findings in the light of fluctuations in task engagement: **Outside of the predictive processing field, attention is often understood as engagement with the task, which may vary according to global variables such as arousal or fatigue. Importantly, our results speak against the proposition that intervals of internal mode processing reflect low task engagement: In addition to shorter RTs and elevated confidence, internal mode was not associated with a general increase in lapse (Supplemental Figures SX), arguing against the proposition that internal mode is associated with random or stereotypical responses that may reflect low task engagement. It is, however, important to note that arousal and attention are known to have non-linear effects on RTs and confidence. Future work should therefore more direct measures of attention and arousal (such as pupil size, motor behavior, or neural data) to better delineate fluctuations in these global variables from fluctuations in the mode of perceptual processing.** 

Following the suggestions of Reviewer 1, we now present our analyses on RT and confidence in a more descriptive way,. We rephrased our discussion of attention in the context of predictive processing, relating attention to the precision of likelihood and prior distribution. Both internal mode and external mode go along with elevated precision in the prior and likelihood, respectively. Thus, both internal and external mode lead to increased precision in the posterior, which provides an explanation for the quadratic or U-shaped relation of mode of confidence.
