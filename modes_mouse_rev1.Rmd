...
Optim_eval = rbind(Optim_eval[,c("value", "type", "free_param", "trial_n", "species", "convcode")], Optim_eval_mice[,c("value", "type", "free_param", "trial_n", "species", "convcode")])
...
p_distribution_Sum <- ggplot(Optim_eval[Optim_eval$convcode == 0,], aes(x = value, color = type, fill = type)) + 
  geom_histogram(fill = "white", position = "dodge") +
  theme_classic(base_size = 6) + xlim(0,1) +
    scale_color_brewer(palette = "Set1", direction = -1) + scale_fill_brewer(palette = "Set1", direction = -1) + 
    theme(legend.position = "top") + facet_wrap(~species) 
p_distribution_Sum
print(Sum)
```
...
We defined the history-biases by comparing the *response about the perceived* stimulus category (A vs. B) at the current and at the preceding trial (choice history). An alternative would be to define history biases by comparing the *presented* stimulus category (stimulus history. We have included a section to the supplement that outlines why our data are better explained by effects of choice history as opposed to stimulus history:

- **In our main manuscript, we investigated the effects of perceptual history, which we defined as the impact of choice at the preceding trial on the choice at the current trial (henceforth *choice history*). Importantly, an alternative approach to this could be to look at *stimulus history*, which is defined as the impact of the stimulus presented at the preceding trial on the choice at the present trial. Here, we compare the effects of choice-history reported in the main manuscript to the effects of stimulus-history in humans and in mice.** 

- **In humans, we found a significant bias toward stimulus history (`r mean_Stimulus_History`% ± `r  error_Stimulus_History`% of trials, $\beta$ = $`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,1]`$ ± $`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,2]`$, T($`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,3]`$) = $`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,4]`$, p = $`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,5]`$) which was smaller than the bias toward choice history ( $\beta$ = $`r STAT.Difference_Stimulus_Choice_History$coefficients[1,1]`$ ± $`r STAT.Difference_Stimulus_Choice_History$coefficients[1,2]`$, T($`r STAT.Difference_Stimulus_Choice_History$coefficients[1,3]`$) = $`r STAT.Difference_Stimulus_Choice_History$coefficients[1,4]`$, p = $`r STAT.Difference_Stimulus_Choice_History$coefficients[1,5]`$).**

- **Logistic regression confirmed the stimulus history made a significant contribution to perception ($\beta$ = $`r STAT.full_logit_Sum$coefficients[3,1]`$ ± $`r STAT.full_logit_Sum$coefficients[3,2]`$, z = $`r STAT.full_logit_Sum$coefficients[3,3]`$, p = $`r STAT.full_logit_Sum$coefficients[3,4]`$) over and above the ongoing stream of current sensory information ($\beta$ = $`r STAT.full_logit_Sum$coefficients[2,1]`$ ± $`r STAT.full_logit_Sum$coefficients[2,2]`$, z = $`r STAT.full_logit_Sum$coefficients[2,3]`$, p = $`r STAT.full_logit_Sum$coefficients[2,4]`$) and general response biases toward one of the two potential outcomes ($\beta$ = $`r STAT.full_logit_Sum$coefficients[1,1]`$ ± $`r STAT.full_logit_Sum$coefficients[1,2]`$, z = $`r STAT.full_logit_Sum$coefficients[1,3]`$, p = $`r STAT.full_logit_Sum$coefficients[1,4]`$). AIC was elevated by $\delta_{AIC}$ = `r -delta_AIC_log_reg_humans` in the stimulus-history model in comparison to the choice history model used in the main manuscript. The above results suggest that the human data were better explained by the effects of choice history as opposed to the effects of stimulus history.** 

- **Like humans, mice showed a significant bias toward stimulus history (`r mean_Stimulus_History`% ± `r  error_Stimulus_History`% of trials, $\beta$ = $`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,1]`$ ± $`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,2]`$, T($`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,3]`$) = $`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,4]`$, p = $`r STAT.Global_Stimulus_History_Accuracy$coefficients[1,5]`$)  which was less pronounced than the bias toward choice history ($\beta$ = $`r STAT.Difference_Stimulus_Choice_History$coefficients[1,1]`$ ± $`r STAT.Difference_Stimulus_Choice_History$coefficients[1,2]`$, T($`r STAT.Difference_Stimulus_Choice_History$coefficients[1,3]`$) = $`r STAT.Difference_Stimulus_Choice_History$coefficients[1,4]`$, p = $`r STAT.Difference_Stimulus_Choice_History$coefficients[1,5]`$).**

- **Logistic regression confirmed the stimulus history made a significant contribution to perception ($\beta$ = $`r STAT.full_logit_Sum$coefficients[3,1]`$ ± $`r STAT.full_logit_Sum$coefficients[3,2]`$, z = $`r STAT.full_logit_Sum$coefficients[3,3]`$, p = $`r STAT.full_logit_Sum$coefficients[3,4]`$) over and above the ongoing stream of current sensory information ($\beta$ = $`r STAT.full_logit_Sum$coefficients[2,1]`$ ± $`r STAT.full_logit_Sum$coefficients[2,2]`$, z = $`r STAT.full_logit_Sum$coefficients[2,3]`$, p = $`r STAT.full_logit_Sum$coefficients[2,4]`$) and general response biases toward one of the two potential outcomes ($\beta$ = $`r STAT.full_logit_Sum$coefficients[1,1]`$ ± $`r STAT.full_logit_Sum$coefficients[1,2]`$, z = $`r STAT.full_logit_Sum$coefficients[1,3]`$, p = $`r STAT.full_logit_Sum$coefficients[1,4]`$). in line with our reusults in humans, AIC was elevated by $\delta_{AIC}$ = `r -delta_AIC_log_reg_humans` in the stimulus-history model in comparison to the choice history-model.** 

- **The above results suggest that, on both humans and mice, our data were better explained by the effects of choice history as opposed to the effects of stimulus history.** 
...
Thanks for important point. Following comment X.X, attention in the predictive coding sense. Conceivable that low task engagement lead to slower responses.

Internal mode and extenral mode both high precision. This translates, as the reviewer notes, to faster evidence accumulation or shift in the starting point. Due to the equivalence between precision in Bayesian models and the mechanistic diffusion models, we feel that we will not benefit from fiting another class of models. 

We realize that the U-Shape of RTs cannot serve as an argument of arousal. We no present our analyses of RT and confidence more descriptive. 

We have rewritten our paragraph on confounds, moving most of the statistics into the supplement. 

We now discuss drift diffusion after the discussion of attention from the perspective of PC (as a mechanistic mapping)
