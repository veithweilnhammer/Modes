...
### Comment 3: Link to reviewer 3, add citation for non-linear arousal effects
...
- The *full model* (model 1) incorporates the following factors: influence of sensory information according to parameter $\alpha$ (likelihood); integration of evidence across trials according to free parameter $H$ (prior); anti-phase oscillations in between the precision of likelihood and prior according to $\omega_{LLR}$ and $omega_{\psi}$ according to parameters $a_{LLR}$ (amplitude likelihood fluctuation), $a_{\psi}$ (amplitude prior fluctuation), $f$ (frequency) and $p$ (phase). This model is the most complex, as it allows prior and likelihood precision to fluctuate at different amplitudes. 

- The *likelihood fluctuation only model* (model 2) incorporates the following factors: influence of sensory information according to parameter $\alpha$ (likelihood); integration of evidence across trials according to free parameter $H$ (prior); oscillations in the precision of likelihood according to $\omega_{LLR}$ with free parameters $a_{LLR}$ (amplitude likelihood fluctuation), $f$ (frequency) and $p$ (phase). 

- The *prior fluctuation only model* (model 3) incorporates the following factors: influence of sensory information according to parameter $\alpha$ (likelihood); integration of evidence across trials according to free parameter $H$ (prior); oscillations in the precision of prior according to $\omega_{\psi}$ with free parameters $a_{\psi}$ (amplitude prior fluctuation), $f$ (frequency) and $p$ (phase). Please note that all models 1-3 lead to shifts in the relative precision of likelihood and prior. 

- The *integration only model* (model 4) incorporates the following factors: influence of sensory information according to parameter $\alpha$ (likelihood); integration of evidence across trials according to free parameter $H$ (prior); no oscillations. This model corresponds to the model proposed by Glaze et al.[@Glaze2015]. This model captures normative evidence accumulation in unpredictable environments using a Bayesian update scheme. Importantly, model 4 thus represents the null hypothesis that fluctuations in mode emerge from a normative Bayesian model without the ad-hoc addition of oscillations as in model 1-3.

- The *no integration model* (model 5) incorporates the following factors: influence of sensory information according to parameter $\alpha$ (likelihood); no integration of evidence across trials; no oscillation. This model has no effective (flat) priors. It represents the null hypothesis that observers do not use prior information derived from serial dependency. 
...
Optim_eval = rbind(Optim_eval, Optim_eval_mice)
Sum <-   ddply(Optim_eval[Optim_eval$convcode == 0,],
                   .(species, type),
                   summarise,
...
In the Results, we now summarize the model comparison and its rationale with a particular focus on the null models 4 and 5 (see also comment X.X.X by Reviewer 3): 
...
### Comment 19: Add citation for arousal, link to reviewer 3, link to response to alternative hypotheses
...
### Comment 22: Done (posterior estimates could be added)
...
### Comment 3: Add model comparison, look for more DDM work on serial choice biases
...
- Limitations and open questions: **The addition of slow anti-phase oscillations to the integration of prior and likelihood represents in the bimodal inference model represents an ad-hoc modification to a normative Bayesian model of evidence accumulation[@Glaze2015]. It is an important task for future research to test whether such slow fluctuations can emerge spontaneously in hierarchical models of Bayesian inference, and whether they can be causally manipulated by expeimental variables. We speculate that between-mode fluctuations may partially separate the perceptual contribution of internal predictions and external sensory data in time, creating unambiguous learning signals that benefit inference about the precision of prior and likelihood, respectively. This proposition should be tested empirically by relating the phenomenon of bimodal inference to performance in, e.g., reversal learning, probabilistic reasoning, or metacognition.** 
