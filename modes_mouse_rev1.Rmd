...
source("./Functions/simulation_glaze_osc_human_zeta_v4.R", local = knitr::knit_global())
...
reproduce_Optim  <- read.csv(paste("./Results/", type_list[type_idx], "_mouse.csv", sep = ""))
...
for (subj_idx in c(1:n_participants)){
  ...
  if (type_list[type_idx] == "fit_glaze_osc_zeta_v4") {

  Sim_add <-
  simulation_glaze_osc_human_zeta_v4(
  outcomes,
  n[subj_idx],
...
The reason for choosing a unique timescale f was to enable our model to depict the the dominant timescale at which prior and likelihood precision are temporarily suspended, giving rise to what we believe constitutes between-mode fluctuations. In line with the shape of the autocorrelation curves, the value for f lies at approximately 0.11 1/$N_{trials}$.   Simulating from our model (Figure 6) replicates the 1/f feature of the empirical data. Please note that the individual trial is the smallest unit of *measurement* for these fluctuations, such that our analysis only deals with frequencies below 1 (1/$N_{trials}$). We have added a paragraph on $f$ to the results section:
...
- explanation of how fluctuations in prior and likelihood precision may impact RTs and confidence: *TODO*

- reference to the potential non-linearity: *TODO*

- reference to future work that will use pupillometry, video tracking and neural signals to discern between-mode fluctuations from global and?or unspecific flucations in performance: *TODO*
...
- attention and precision in PC: *TODO*
...
Allowing the learning rate itself to vary as a function of preceding experiences would add an additional level of complexity that we sought to omit in this analysis. However, we fully agree that choosing a Bayesian framework (e.g., a three-level HGF) would indeed be
more consistent.
