    ...
    if (nrow(Input_Data)  > block_size) {
      ...
      #for (trial_idx  in c(1)) {
        ...
        #print(type_list[type_idx])
          ...
          Optim_eval = rbind(Optim_eval, add_O[, c('value', 'AIC', 'type', 'free_param')])
          #Optim_eval = rbind(Optim_eval, add_O)
        }
      
    }
    }
    write.csv(Optim_eval, "./Results/Optim_eval_human_interim.csv", row.names = FALSE)
}
...
for (id in min(MwData$subject_id)) {
  ...
  for (session_id in min(MwData[MwData$subject_id == id, ]$session_id)) {
    ...
    write.csv(Optim_eval, "./Results/Optim_eval_mouse_interim.csv", row.names = FALSE)
  }
}
...
```{r random_walk, cache = TRUE}
pos <- sigmoid(0 + cumsum(rnorm(200, sd = 0.1)))
...
- **To allow for *bimodal inference*, i..e, alternating periods of internally- and externally-biased modes of perceptual processing that occur irrespective of the sequence of preceding experiences, we assumed that the relative influence of likelihood and prior vary according to fluctuations governed by $\omega_{LLR}$, i.e., a sine function (defined by amplitude $a$, frequency $f$ and phase $p$) that determines the precision afforded to the likelihood[@Feldman2010]. This implements a hyperprior[@Friston2005] in which the likelihood and prior precisions are shifted against each other at a dominant timescale defined by $f$: (...)**
...
- We present our results on the relation of mode to RT and Confidence in a more descriptive way, and do not use it as a strong defensive analysis against attention, arousal, fatigue or task engagement: *To Do*.
...
We feel that this point is closely related to the comment above. We realize that, in the predictive coding field, attention is equated with the precision of probability distributions that contribute to the perceptual decision, such that an observer can attend strongly to sensory information (high likelihood precision) or to internal predictions derived from the sequence of preceding percepts (high prior precision). Therefore, when following the above predictive coding definition, fluctuations in attention can be equated with fluctuations in mode. 
...
We now provide a more nuanced interpretation of our findings of RTs and confidence in relation to attention, with a specific focus on predictive coding and precision (same as above):
...
- Prior fluctuation only (model 3): influence of sensory information according to parameter $\alpha$ (likelihood); integration of evidence across trials according to free parameter $H$ (prior); oscillations in the precision of prior according to $\omega_{\psi}$ with free parameters $a_{\psi}$ (amplitude prior fluctuation), $f$ (frequency) and $p$ (phase). Please note that all models 1-3 lead to shifts in the relative precision of likelihood and prior. 
...
We have followed this suggestion and move the following sections to the supplement (*To Do*):

- We now present the relation of mode to RTs and confidence in the context of predictive processing accounts of attention (see points below). The additional defensive analysis from section 5.3 (Internal and external modes of processing facilitate response behavior and enhance confidence in human perceptual decision-making) have been moved to the supplement. 

- We moved section 5.4 (Fluctuations between internal and external mode modulate perceptual performance beyond the effect of general response biases) to the supplement. 

- We summarize the upshot of section 5.5 (Internal mode is characterized by lower thresholds as well as by history-dependent changes in biases and lapses), since it provides a test against the null hypothesis that what we identify as internal mode processing is indeed periods of random or impulsive choices: **Text**. The statistics on the psychmetric functions have been moved to the supplement. 

- We moved the corresponding paragraphs from the section on mouse behavior into the supplement. 
...
Thank you very much for pointing this out. We have adapted the manuscript to make the distinction between attention in the predictive processing sense and the broader issue of task engagement (reflecting fluctuations in arousal, fatigue etc.) clearer (see also comments above): 

 **According to predictive processing, attention corresponds to the precision afforded to the probability distributions that underlie perceptual inference[@Feldman2010]. As outlined above, between-mode fluctuations can be understood as ongoing shifts in the precision afforded to likelihood (*external mode*) and prior (*internal mode*), respectively. When the precision afforded to prior or likelihood increases, posterior precision increases, which leads to faster RTs and higher confidence. When defined from the perspective of predictive processing as the precision afforded to likelihood and prior[@Feldman2010], fluctuations in attention may thus provide a plausible explanation for the quadratic relationship between mode and RTs and confidence (Figure 2H and J; Figure 4I, Figure 6I)**.

**Outside of the predictive processing field, attention is often equated as engagement with the task, which may vary according to global variables such as arousal or fatigue. Importantly, our results speak against the proposition that intervals of internal mode processing reflect low task engagement: In addition to shorter RTs and elevated confidence, internal mode was not associated with a general increase in lapse (Supplemental Figures SX), arguing against the proposition that internal mode is associated with random or stereotypical responses that may reflect low task engagement. It is, however, important to note that arousal and attention are known to have non-linear effects on RTs and confidence. Future work should therefore more direct measures of attention and arousal (such as pupil size, motor behavior, or neural data) to better delineate fluctuations in these global variables from fluctuations in the mode of perceptual processing.** 
...
We would like to thank the reviewer for this suggestion. We added this to the discussion of our model in the discussion:
